...
bool DriveTrain::driveStraight(double distance, double angleToMaintain) {
    ...
    while(abs(distanceError) > Distance::THRESHOLD) {
      ...
      if(LocationManager::obstaclesBlockingTheFront()) {
        ...
        TankSimulation.turnOffMotors(); //TODO: REPLACE WITH JUST STOP() AFTER FINISHING SIMULATION
        return false;
      }
    }
...
void DriveTrain::smartDrive(double speed, double angularError) {
  ...
  TankSimulation.setLeftMotorPWM(speed * 255 + angleSpeedAdjustment);
  TankSimulation.setRightMotorPWM(speed * 255 - angleSpeedAdjustment);
}
